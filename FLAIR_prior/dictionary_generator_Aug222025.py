...
class IVIMProcessor:
    ...
    def __init__(self):
        ...
        self.path_templates = {
            ...
            "FA": "/content/drive/MyDrive/IVIM_NeuroCovid/subject_folder/{SUBJECTID}/FA2ivim.nii.gz",
            "MD": "/content/drive/MyDrive/IVIM_NeuroCovid/subject_folder/{SUBJECTID}/MD2ivim.nii.gz"
        }
    ...
    def load_and_validate_data(self, paths: Dict[str, str]) -> Dict[str, np.ndarray]:
        ...
        data = {
            ...
            "FA": nib.load(paths["FA"]).get_fdata().astype(np.float32),
            "MD": nib.load(paths["MD"]).get_fdata().astype(np.float32),
            "bvals": bvals_final
        }

        # Validate shapes (simplified - just check first 3 dimensions)
        ref_shape = data["ivim"].shape[:3]
        for key in ["b0avg", "flair", "tissue", "lesion", "FA", "MD"]:
    ...
    def process_subject(self, subject_id: str, n_per_class: int = 500, seed: int = 69) -> Tuple[pd.DataFrame, np.ndarray, np.ndarray]:
        ...
        # Flatten maps
        ivim_flat = data["ivim"].reshape(-1, T) 
        b0avg_flat = data["b0avg"].flatten()
        flair_flat = data["flair"].flatten()
        tissue_flat = data["tissue"].flatten()
        lesion_flat = data["lesion"].flatten()
        T1_flat = data["T1"].flatten()
        FA_flat = data["FA"].flatten()
        MD_flat = data["MD"].flatten()
        ...
        df = pd.DataFrame({
            ...
            "tissue_lesion_label": labels_str[selected_flat_indices],
            "b0_avg": b0avg_flat[selected_flat_indices],
            "flair_signal": flair_flat[selected_flat_indices],
            "T1_signal": T1_flat[selected_flat_indices],
            "FA": FA_flat[selected_flat_indices],
            "MD": MD_flat[selected_flat_indices]
        })
    ...
    def process_multiple_subjects(self, subject_ids: List[str], output_dir: str,
        ...
        np.savez_compressed(
            ...
            FA=combined_df["FA"].values,
            MD=combined_df["MD"].values,

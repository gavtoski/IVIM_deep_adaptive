...
def classify_tissue_by_signal(signal_val, model_type="3C", IR=False, custom_bounds_dict=None):
	...
	Uses closest mean if overlapping bounds or no match.

	Args:
		signal_val (float): Normalized b=1000 signal.
		model_type (str): '3C' or '2C'.
		IR (bool): IR mode flag (3C only).
		custom_bounds_dict (dict): Optional {tissue: (mean, std)} overrides.

	Returns:
		str: 'NAWM', 'WMH', or 'S1' label.
	"""
	...
	bounds_dict = custom_bounds_dict if custom_bounds_dict else default_bounds[model_type][IR]

	# Build bounds
	bounds = {k: (v[0] - v[1], v[0] + v[1]) for k, v in bounds_dict.items()}

	# Candidates where signal falls inside mean ± std
	candidates = [k for k, (low, high) in bounds.items() if low <= signal_val <= high]

	if len(candidates) == 1:
		return candidates[0]
	elif len(candidates) > 1:
		# Pick closest mean if multiple candidates
		return min(candidates, key=lambda k: abs(signal_val - bounds_dict[k][0]))
	else:
